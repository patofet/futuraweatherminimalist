...
void weather_layer_set_temperature(WeatherLayer* weather_layer, int16_t t, bool error) {
  WeatherLayerData *wld = layer_get_data(weather_layer);
  if(error)
    snprintf(wld->temp_str, sizeof(wld->temp_str), "--%s", "°");
  else
    snprintf(wld->temp_str, sizeof(wld->temp_str), "%i%s", t,"°");
